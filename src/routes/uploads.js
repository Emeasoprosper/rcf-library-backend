...
// Resolves one or more categories for a new resource. Accepts the NEW
// comma-separated categoryNames field ("Christian, Personal Growth") —
// each name is found-or-created (same case-insensitive dedupe as the
// existing POST /meta/categories route) — AND still honors the OLD
// single categoryId field for as long as the upload form keeps sending
// it, merging both in if present rather than one silently overriding
// the other. This keeps every existing upload flow working unchanged
// while the new multi-category table (resource_categories) becomes the
// real source of truth going forward.
async function resolveCategoryIds({ categoryId, categoryNames }) {
  const ids = []
  if (categoryNames) {
    const names = String(categoryNames).split(',').map((c) => c.trim()).filter(Boolean).slice(0, 8)
    for (const name of names) {
      const existing = await query('SELECT id FROM categories WHERE name ILIKE $1', [name])
      const id = existing.rows[0]?.id
        || (await query('INSERT INTO categories (name) VALUES ($1) RETURNING id', [name])).rows[0].id
      if (!ids.includes(id)) ids.push(id)
    }
  }
  if (categoryId && !ids.includes(Number(categoryId))) {
...
router.post(
  ...
  async (req, res) => {
    ...
    const {
      ...
      categoryId, categoryNames, departmentId, level, semester, isAnonymous, mediaSubtype, tags,
      chapter, part, volume, edition, collectionId,
    } = req.body

    const categoryIds = await resolveCategoryIds({ categoryId, categoryNames })
    ...
    const rawFilenameTitle = uploadedFile.originalname
      .replace(/\.[^./\\]+$/, '')
      .replace(/[-_]+/g, ' ')
      .trim()
    ...
    const inserted = await query(
      ...
      [
        ...
        finalDepartmentId, primaryCategoryId, typeResult.rows[0].id, finalLevel, semester || null,
    ...
    for (const catId of categoryIds) {
      await query(
        'INSERT INTO resource_categories (resource_id, category_id) VALUES ($1, $2) ON CONFLICT DO NOTHING',
        [resourceId, catId]
      )
    }
